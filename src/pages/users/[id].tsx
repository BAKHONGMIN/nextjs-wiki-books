        ...
        <Box width="1180">
          ...
          <Box>
            <Box marginBottom={1}>
              {/* 
                사용자 프로파일 컨테이너
                사용자 점보를 표시한다. useUser로 항상 최신 데이터를 얻는다.
              */}
            ...
            {/* 
              사용자 상품 카드 리스트 컨테이너
              사용자가 소유한 상품 카드 리스트를 표시한다. useSearch로 항상 최신 데이터를 얻는다.
            */}
...
export const getStaticPaths: GetStaticPaths = async () => {
  const context: ApiContext = {
    apiRootUrl: process.env.NEXT_BASE_URL || "http://localhost:5000",
  }
...
export const getStaticProps = async ({ params }: GetStaticPropsContext) => {
  ...
  //사용자 정보가 사용자가 소유한 상품을 얻고, 정적 페이지를 생성한다
  //10초 동안 revalidate 상태로 하고, 정적 페이지를 업데이트 한다.
